...
end)  -- [[ NERO HUB: Módulo de Câmera (Mobile / Bypass) - V3 ]] --
task.spawn(function()
    ...
    -- BOTÃO INVISÍVEL (Hitbox gigante para celular: 60x60)
    local hitbox = Instance.new("TextButton")
    hitbox.Name = "TouchArea"
    hitbox.Size = UDim2.new(0, 60, 0, 60)
    hitbox.Position = UDim2.new(0.5, -30, 0, 0) -- Colado no topo, no meio
    hitbox.BackgroundTransparency = 1 -- Totalmente invisível
    hitbox.Text = ""
    hitbox.Parent = neroGui

    -- GRUPO VISUAL (O botão real de 20x20 que você vê)
    local visualGroup = Instance.new("CanvasGroup")
    visualGroup.Size = UDim2.new(0, 20, 0, 20)
    visualGroup.Position = UDim2.new(0.5, 0, 0, 12)
    visualGroup.AnchorPoint = Vector2.new(0.5, 0.5)
    visualGroup.BackgroundColor3 = neroBlack
    visualGroup.GroupTransparency = 0.9 -- Começa 10% visível
    visualGroup.Parent = hitbox
    ...
    local function triggerActiveAnim()
        ...
        TweenService:Create(visualGroup, animFast, {GroupTransparency = 0}):Play() -- Fica 100% visível
        
        task.delay(2.1, function()
            if tick() - lastInteraction >= 2.0 then
                -- Se passou 2 segundos sem toques, volta para 10% (0.9 de transparência)
                TweenService:Create(visualGroup, animSlow, {GroupTransparency = 0.9}):Play()
            end
        end)
    end

    -- Sistema de Câmera (Bypass permitindo movimento de tela)
    ...
    local function setThirdPersonBypass(state)
        ...
        if state then
            -- Roda o cálculo depois que a câmera do jogo atualizar, mantendo o controle do seu dedo (rotação)
            RunService:BindToRenderStep("NeroCamBypass", Enum.RenderPriority.Camera.Value + 1, function()
                local char = player.Character
                if char and char:FindFirstChild("HumanoidRootPart") then
                    local hrp = char.HumanoidRootPart
                    -- Pega a posição do jogador, mas usa a rotação que VOCÊ controla, jogando 10 studs pra trás
    ...
    local function setFixedCamera(state)
        cameraFixed = state
        if state then
            -- Trava o boneco pra olhar na mesma direção da câmera (estilo shift-lock)
    ...
    -- Lógica de Toques (1 Toque vs 2 Toques)
    local tapCount = 0
    local doubleTapTime = 0.35 -- Aumentei um pouco para facilitar o toque duplo no celular

    -- Usando .Activated (muito melhor para touch screen que MouseButton1Click)
